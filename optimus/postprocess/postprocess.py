...
def create_grid(
    ...
    plane_axes=DP.PLANE_AXES,
    plane_offset=DP.PLANE_OFFSET,
    ...
    **points
    ...
    mode: string
        The values are 'structured' and 'unstructured'. The unstructured grids are generated by gmsh, necessary for plotting both 3D surface and 2D plane values.
    **points : np.array of 3xN
        A numpy array of spacial points for visualisation.

    Output
    ----------
    vis_points: Visualisation grid
    ...
    if points:
        ...
        warnings.warn(
            "User-defined visualisation points. Ignored grid properties: resolution, bounding_box, etc. ",
            RuntimeWarning,
        )
        vis_points = points
    else:
        ...
        if mode == "structured":
            ...
            vis_points = np.vstack((points_tmp,))
            ...
            vis_points = plane_grid.leaf_view.vertices
    return vis_points
    ...
    def visualiser(
        ...
        (
            ...
            idx_interior,
            idx_exterior,
        ) = self.create_grid(system_setup, mode=mode, user_grid=user_grid)
        ...
        incident_exterior = incident_field(
            transducer_parameters, system_setup, points_exterior
        )
        ...
        if mode == "2D":
            ...
            domain_solutions_all = [
                solution[2 * i].coefficients
                for i in range(system_setup.number_of_scatterers)
            ]
