...
class PostProcess(object):
    ...
    def solution_to_gridfunction(self, model, gmres_solution):
        ...
        for spaces in self.chunker(model.operator_blocks.domain_spaces, 2):
            ...
            gmres_solution_index[1] = gmres_solution_index[0] + spaces[domain_spaces_index[0]].global_dof_count
            gmres_solution_index[2] = gmres_solution_index[1] + spaces[domain_spaces_index[1]].global_dof_count

            scatterer_gridfunction = [
                bempp.GridFunction(spaces[domain_spaces_index[0]],
                                   coefficients=gmres_solution[gmres_solution_index[0]:gmres_solution_index[1]]),
                bempp.GridFunction(spaces[domain_spaces_index[1]],
                                   coefficients=gmres_solution[gmres_solution_index[1]:gmres_solution_index[2]])
            ]
    ...
    def plane_grid(self,
        ...
        if sys.version_info.major >= 3 and sys.version_info.minor >= 6:
            ...
            geometry = ("ax1_lim1 = " + str(x_axis_lims[0]) + ";\n" + "ax1_lim2 = " + str(x_axis_lims[1]) + ";\n" +
                        "ax2_lim1 = " + str(y_axis_lims[0]) + ";\n" + "ax2_lim2 = " + str(y_axis_lims[1]) + ";\n" +
                        "rot_ax1 = " + str(rotation_axis[0]) + ";\n" + "rot_ax2 = " + str(rotation_axis[1]) + ";\n" +
                        "rot_ax3 = " + str(rotation_axis[2]) + ";\n" + "rot_ang_rad = " + rotation_angle + ";\n" +
                        "cl = " + str(h) + ";\n" + stub)
        return self.generate_grid_from_geo_string(geometry)

    def create_grid(self, system_setup, mode=DP.CREATE_GRID_MODE, user_grid=False):

        from .exterior_interior_points_eval import exterior_interior_points_eval

        self.number_of_scatterers = system_setup.global_parameters.number_of_scatterers

        if not self.bounding_box:
            ax1_min, ax1_max, ax2_min, ax2_max = 0, 0, 0, 0

            for grid in system_setup.grids:

                ax1_min = np.min([ax1_min, grid.bounding_box[0][self.plane_axes[0]].min()])
                ax1_max = np.max([ax1_max, grid.bounding_box[1][self.plane_axes[0]].max()])
                ax2_min = np.min([ax2_min, grid.bounding_box[0][self.plane_axes[1]].min()])
                ax2_max = np.max([ax2_max, grid.bounding_box[1][self.plane_axes[1]].max()])

            self.bounding_box = [5. * x for x in [ax1_min, ax1_max, ax2_min, ax2_max]]
        ...
        if (not user_grid and (self.obj_changed or not self.points.any())):
            ...
            if mode == '2D':
                plot_grid = np.mgrid[ax1_min:ax1_max:self.resolution[0] * 1j, ax2_min:ax2_max:self.resolution[1] * 1j]
                ...
                elem_len = np.min([(axis1_lims[1] - axis1_lims[0]) / self.resolution[0],
                                   (axis2_lims[1] - axis2_lims[0]) / self.resolution[1]])

                plane_grid = self.plane_grid(axis1_lims, axis2_lims, rotation_axis, rotation_angle, elem_len)
            ...
            if system_setup.import_mesh:
                for grid in system_setup.grids:
                    POINTS_INTERIOR_TMP, POINTS_EXTERIOR_TMP, IDX_INTERIOR_TMP, IDX_EXTERIOR_TMP = exterior_interior_points_eval(
                        grid,
                        points)
                ...
                for x,y,z,R0 in zip(system_setup.x_center, system_setup.y_center, system_setup.z_center,system_setup.R0_list):
            ...
            assert isinstance(self.index_interior, list), "index_interior must be a list."
            assert isinstance(self.index_exterior, list), "index_exterior must be a list."
        ...
        return self.points, self.points_interior, self.points_exterior, self.index_interior, self.index_exterior

    def visualiser(self, solution, system_setup, transducer_parameters, h_mat='dense', mode='2D', user_grid=False):

        from ..source.source_field import incident_field
        ...
        self._number_of_scatterers = system_setup.global_parameters.number_of_scatterers

        points, points_interior, points_exterior, idx_interior, idx_exterior = self.create_grid(system_setup,
                                                                                                mode = mode,
                                                                                                user_grid = user_grid)
        ...
        incident_exterior = incident_field(transducer_parameters, system_setup, points_exterior)

        if self.verbose:
            TE_INC_FIELD = time.time()
            print('\n Calculating the incident field Finished... Duration in secs: ', TE_INC_FIELD - TS_INC_FIELD)
        ...
        for (solution_pair, interior_point, interior_density, interior_id,
             interior_wavenumber) in zip(self.chunker(solution, 2),
                                         points_interior,
                                         system_setup.interior_properties.density,
                                         idx_interior,
                                         system_setup.interior_properties.wavenumber):
            if self.verbose:
                print('Calculating the fields of Domain {0}'.format(i + 1))
                print(interior_point.shape, interior_id.shape, interior_wavenumber)

            pot_int_sl = bempp.operators.potential.helmholtz.single_layer(solution_pair[1].space,
                                                                          interior_point,
                                                                          interior_wavenumber)
            pot_int_dl = bempp.operators.potential.helmholtz.double_layer(solution_pair[0].space,
                                                                          interior_point,
                                                                          interior_wavenumber)
            pot_ext_sl = bempp.operators.potential.helmholtz.single_layer(solution_pair[1].space,
                                                                          points_exterior,
                                                                          system_setup.exterior_properties.wavenumber)
            pot_ext_dl = bempp.operators.potential.helmholtz.double_layer(solution_pair[0].space,
                                                                          points_exterior,
                                                                          system_setup.exterior_properties.wavenumber)

            rho_ratio = interior_density / system_setup.exterior_properties.density

            exterior_values += -pot_ext_sl * solution_pair[1] + pot_ext_dl * solution_pair[0]
            interior_value = pot_int_sl * solution_pair[1] * rho_ratio - pot_int_dl * solution_pair[0]
        ...
        incident_exterior_field[idx_exterior] = incident_exterior.pressure.ravel()

        total_field[idx_exterior] = scattered_field[idx_exterior] + incident_exterior_field[idx_exterior]
        ...
        if mode == '2D':
            ...
            self.incident_field = incident_exterior_field.reshape(self.resolution, order='F')
            self.l2_norm_total_field_mpa = np.linalg.norm(total_field)

        elif mode == '3D':
            domain_grids = system_setup.grids
            ...
            domain_solutions_all = [solution[2 * i].coefficients for i in range(system_setup.number_of_scatterers)]
            domain_solutions_all.append(total_field)
            plot3D_ptot_all = bempp.GridFunction(space_union_all,
                                                 coefficients=np.concatenate([
                                                     domain_solutions_all[i]
                                                     for i in range(system_setup.number_of_scatterers + 1)
                                                 ]))
            plot3D_ptot_abs_all = bempp.GridFunction(space_union_all,
                                                     coefficients=np.concatenate([
            ...
            bempp.export(file_name=DP.PTOT_ABS_ALL_FILENAME, grid_function=plot3D_ptot_abs_all)
    ...
    def ppi_calculator(self):
        diagonal_length_meter = np.sqrt((self.bounding_box[1] - self.bounding_box[0])**2 +
    ...
    def print_parameters(self):
        ...
        print('\n bounding box (2D frame) points: ', [self.bounding_box[0], self.bounding_box[2]],
    ...
    def domain_edges(self, alpha=0.005, only_outer=True):
        ...
        for k in range(self.number_of_scatterers):
            ...
            for i, j in edges:
                self.edge_points.append(np.vstack([points_int[0, [i, j]], points_int[1, [i, j]]]))
