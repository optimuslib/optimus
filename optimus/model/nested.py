    ...
    def assemble_boundary_integral_operators(self):
        ...
        def assemble(continuous_operators_oneway):
            ...
            if all([prec == "none" for prec in self.preconditioners]):
                ...
                [prec in ("mass", "osrc", "calderon") for prec in self.preconditioners]
            ):
                for key, operator in continuous_operators_oneway.items():
                    discrete_operators[key] = operator.strong_form()
            else:
                raise ValueError(
                    "Unknown preconditioner settings: "
                    + self.preconditioners[0]
                    + ", "
...
class PreconditionerOperators:
    ...
    def _apply_calderon_operators(self, vector):
        ...
        if self.formulation == "pmchwt":
            ...
            result = _np.concatenate(
                [-op_dl * vec_dir + op_sl * vec_neu, op_hs * vec_dir + op_ad * vec_neu]
            )
        ...
        if self.formulation in ("pmchwt", "muller", "multitrace"):
            ...
            if self.formulation in ("pmchwt", "muller"):
                ...
                source_vector = _np.concatenate(
                    [vec_dir, vec_neu, _np.zeros_like(vec_dir), _np.zeros_like(vec_neu)]
                )
            else:
                raise AssertionError("Unknown formulation: " + self.formulation)
...
def _split_trace_vector(vector, formulation, space):
    ...
    if formulation in ("pmchwt", "muller"):
        ...
        trace_vectors = _np.split(vector, indices_or_sections=_np.cumsum([ndof] * 3))
        traces = {
            "dirichlet-exterior": trace_vectors[0],
            "neumann-exterior": trace_vectors[1],
        ...
        raise AssertionError("Unknown formulation: " + formulation)
